...
function addGamesToPage(games) {
    ...
    games.forEach(game => {
        ...
        card.innerHTML = `<img src="${game.img}" alt="game image" class = "game-img">
        <p>The game ${game.name} currently has ${game.backers} backers.</p>`;
...
function filterUnfundedOnly() {
    ...
    const filtered_games = GAMES_JSON.filter((game) => {
...
const unfunded_games = GAMES_JSON.reduce((acc, game) => {
    let val = game.pledged < game.goal ? 1 : 0;
    return acc + val;
}, 0);


// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${total_money.toLocaleString('en-US')} has been raised for ${total_games.toLocaleString('en-US')}. 
Currently, ${unfunded_games.toLocaleString('en-US')} ${unfunded_games == 1 ? "game" : "games"}
...
first_p.innerHTML = `${first_game.name.toLocaleString('en-US')}`;
firstGameContainer.appendChild(first_p);

// do the same for the runner up item
const second_p = document.createElement("p");
second_p.innerHTML = `${second_game.name.toLocaleString('en-US')}`;
secondGameContainer.appendChild(second_p);